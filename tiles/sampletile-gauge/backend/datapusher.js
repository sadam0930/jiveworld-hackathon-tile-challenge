...
function processTileInstance(instance) {
    ...
    var statusMessage = "purple button"
    var status = ""; 
    var newLevel;
    
    //console.log("btnPushed = " + btnPushed);
    if (btnPushed) {
      status = statusMessage;
      newLevel = getRandomInt(0,4);
    } else {
      status = statusNames[level];
      newLevel = instance.config.level;
    }

    var level = Number(newLevel);
    var dataToPush = {
        data: {
...
function pushData() {
    ...
    jive.tiles.findByDefinitionName( 'sampletile-gauge' ).then( function(instances) {
        if ( instances ) {
